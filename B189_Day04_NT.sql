...
SELECT calisan_id || ' - ' || calisan_isim AS id_isim FROM workers;--concationation 

SELECT * FROM workers AS calisanlar;--Bir tablo ismi icin gecici isim koymak istedigin zama nonun isminin ilk harfi kullanilir BP 
...
--marka-id si 100 olan markada calisanlari listeleyiniz
...
WHERE isyeri=(SELECT marka_isim
			  FROM markalar
			  WHERE marka_id = 100);--parantez ici subquery

--Interview question : Calisanlar3 tablosunda max maasi alan calisanin tum fieldlarini listeleyiniz

SeLECT *
FROM calisanlar3 WHERE maas = (SELECT MAX(maas)  FROM calisanlar3)

--interview quest : calisanlar3 tablosunda ikinci en yuksek maasi gosteriniz , ODEV

SELECT MAX(maas)
FROM calisanlar3
WHERE maas<(SELECT MAX(maas) FROM calisanlar3)

--calisanlar3 tablosunda ikinci en yuksek maasi alan calisani gosteriniz
...
--calisanlar tablosunda max veya min maasi alan calisanlarin tum fieldlarini gosteriniz
